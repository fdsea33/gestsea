...
CREATE OR REPLACE VIEW VUE_Historique AS
  SELECT distinct on (p.pe_numero) ct_nom as "Canton", 
       ...
       CASE WHEN p.pe_numero IN (select pe_numero from table_cotisation where cs_annee>=EXTRACT(YEAR FROM CURRENT_DATE)-1 AND BML_EXTRACT(cs_detail,'fdsea.forfait.produit')::integer IN (300006,500000052,500000162)) THEN 'X' ELSE '' END AS "Exploitant",
