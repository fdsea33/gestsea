...
CREATE OR REPLACE VIEW VUE_Historique2 AS
  SELECT distinct on (p.pe_numero) ct_nom as "Canton", 
       ...
       pe_nom as "Nom", 
       INITCAP(pe_prenom) as "Prenom", 
       CASE WHEN p.pe_numero IN (select pe_numero from table_cotisation where cs_annee>=EXTRACT(YEAR FROM CURRENT_DATE)-2 AND BML_EXTRACT(cs_detail,'fdsea.forfait.produit')::integer IN (300006,500000052)) THEN 'X' ELSE '' END AS "Exploitant",
       CASE WHEN p.pe_numero IN (select pe_numero from table_cotisation where cs_annee>=EXTRACT(YEAR FROM CURRENT_DATE)-2 AND BML_EXTRACT(cs_detail,'fdsea.forfait.produit')::integer IN (500000124,500000053)) THEN 'X' ELSE '' END AS "Bailleur",
       CASE WHEN p.pe_numero IN (select pe_numero from table_cotisation where cs_annee>=EXTRACT(YEAR FROM CURRENT_DATE)-2 AND BML_EXTRACT(cs_detail,'fdsea.forfait.produit')::integer IN (500000054,500000124,500000150)) THEN 'X' ELSE '' END AS "Ancien",
