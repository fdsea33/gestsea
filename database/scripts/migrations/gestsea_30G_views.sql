...
CREATE OR REPLACE VIEW "personne" AS
   SELECT DISTINCT ON (table_personne.pe_numero) table_personne.pe_numero, table_personne.pe_id, table_personne.tp_numero, table_personne.np_numero, table_personne.pe_titre, table_personne.pe_nom, table_personne.pe_regimefiscal, table_personne.ep_numero, table_personne.pe_morale, table_personne.pe_prenom, table_personne.pe_motdepasse, table_personne.pe_naissance, table_personne.pe_numtvaic, table_personne.created_at, table_personne.created_by, table_personne.updated_at, table_personne.updated_by, table_personne.lock_version, table_personne.id, TRIM(COALESCE(PE_Titre||' ','')|| COALESCE(PE_Nom,'')|| COALESCE(' '||PE_Prenom,'')) AS PE_Libelle, COALESCE(PE_Nom||' ','')|| COALESCE(PE_Prenom,'')||COALESCE(' ('||NULLIF(TRIM(PE_Titre),'')||')','') AS PE_Fullname, to_char(PE_ID,'FM099999') AS PE_NumPersonne, Tel.CN_Coordonnee AS PE_telephone, Fax.CN_Coordonnee AS PE_Fax, CT_Nom, COALESCE(PE_Titre,'')||';'|| COALESCE(PE_Nom,'')||';'|| COALESCE(PE_Prenom,'')||';'||COALESCE(AD_Ligne2,'')||';'|| COALESCE(Ad_Ligne3,'')||';'|| COALESCE(Ad_Ligne4,'')||';'|| COALESCE(Ad_Ligne5,'')||';'|| CP_CodePostal||';'|| VI_Nom AS PE_Adresse, VI_Nom AS PE_Ville, CP_CodePostal AS PE_CP, CT_Nom AS PE_Canton, to_char(PE_ID,'FM099999')||' - '||TRIM(COALESCE(PE_Titre||' ','')|| COALESCE(PE_Nom,'')|| COALESCE(' '||PE_Prenom,''))||' ('||COALESCE(CP_CodePostal,0)||' '||COALESCE(vi_nom,'???')||')' AS PE_Description 
...
CREATE OR REPLACE VIEW "facture" AS
   SELECT table_facture.fa_numero, table_facture.de_numero, table_facture.pe_numero, table_facture.ag_numero, table_facture.fa_numfact, table_facture.fa_date, ROUND(table_facture.fa_reduction,2) AS fa_reduction, ROUND(table_facture.fa_montantht,2) AS fa_montantht, ROUND(table_facture.fa_montantttc,2) AS fa_montantttc, ROUND(table_facture.fa_accompte,2) AS fa_accompte, table_facture.fa_annotation, table_facture.fa_libelle, table_facture.so_numero, table_facture.created_at, table_facture.created_by, table_facture.updated_at, table_facture.updated_by, table_facture.lock_version, table_facture.id 
     FROM "table_facture" 
    WHERE SO_Numero IN (SELECT SE_Societe FROM VUE_CURRENT_Societe);
...
CREATE OR REPLACE RULE rule_facture_insert AS
  ...
  DO INSTEAD INSERT INTO "table_facture"(fa_numero, de_numero, pe_numero, ag_numero, fa_numfact, fa_date, fa_reduction, fa_montantht, fa_montantttc, fa_accompte, fa_annotation, fa_libelle, so_numero, created_at, created_by, updated_at, updated_by, lock_version, id) VALUES (new.fa_numero, new.de_numero, new.pe_numero, new.ag_numero, new.fa_numfact, COALESCE(NEW.fa_date,CURRENT_DATE), ROUND(COALESCE(NEW.fa_reduction,0),2), ROUND(COALESCE(NEW.fa_montantht,0),2), ROUND(COALESCE(NEW.fa_montantttc,0),2), ROUND(new.fa_accompte,2), new.fa_annotation, new.fa_libelle, COALESCE(NEW.so_numero,current_societe()), CURRENT_TIMESTAMP, CURRENT_USER, CURRENT_TIMESTAMP, CURRENT_USER, 0, DEFAULT);
CREATE OR REPLACE RULE rule_facture_update AS
  ON UPDATE TO "facture"
  DO INSTEAD UPDATE "table_facture" SET fa_numero=new.fa_numero, de_numero=new.de_numero, pe_numero=new.pe_numero, ag_numero=new.ag_numero, fa_numfact=new.fa_numfact, fa_date=COALESCE(NEW.fa_date,CURRENT_DATE), fa_reduction=ROUND(COALESCE(NEW.fa_reduction,0),2), fa_montantht=ROUND(COALESCE(NEW.fa_montantht,0),2), fa_montantttc=ROUND(COALESCE(NEW.fa_montantttc,0),2), fa_accompte=ROUND(new.fa_accompte,2), fa_annotation=new.fa_annotation, fa_libelle=new.fa_libelle, so_numero=COALESCE(NEW.so_numero,current_societe()), created_at=OLD.created_at, created_by=OLD.created_by, updated_at=CURRENT_TIMESTAMP, updated_by=CURRENT_USER, lock_version=OLD.lock_version+1, id=OLD.id WHERE new.FA_Numero=FA_Numero;
...
CREATE OR REPLACE RULE rule_personne_insert AS
  ...
  DO INSTEAD INSERT INTO "table_personne"(pe_numero, pe_id, tp_numero, np_numero, pe_titre, pe_nom, pe_regimefiscal, ep_numero, pe_morale, pe_prenom, pe_motdepasse, pe_naissance, pe_numtvaic, created_at, created_by, updated_at, updated_by, lock_version, id) VALUES (new.pe_numero, DEFAULT, new.tp_numero, new.np_numero, new.pe_titre, new.pe_nom, COALESCE(NEW.pe_regimefiscal,'NON RENSEIGNE'), new.ep_numero, COALESCE(NEW.pe_morale,false), new.pe_prenom, new.pe_motdepasse, new.pe_naissance, new.pe_numtvaic, CURRENT_TIMESTAMP, CURRENT_USER, CURRENT_TIMESTAMP, CURRENT_USER, 0, DEFAULT);
CREATE OR REPLACE RULE rule_personne_update AS
  ON UPDATE TO "personne"
  DO INSTEAD UPDATE "table_personne" SET pe_numero=new.pe_numero, pe_id=OLD.PE_ID, tp_numero=new.tp_numero, np_numero=new.np_numero, pe_titre=new.pe_titre, pe_nom=new.pe_nom, pe_regimefiscal=COALESCE(NEW.pe_regimefiscal,'NON RENSEIGNE'), ep_numero=new.ep_numero, pe_morale=COALESCE(NEW.pe_morale,false), pe_prenom=new.pe_prenom, pe_motdepasse=new.pe_motdepasse, pe_naissance=new.pe_naissance, pe_numtvaic=new.pe_numtvaic, created_at=OLD.created_at, created_by=OLD.created_by, updated_at=CURRENT_TIMESTAMP, updated_by=CURRENT_USER, lock_version=OLD.lock_version+1, id=OLD.id WHERE new.PE_Numero=PE_Numero;
