  ...
  def migrate(debug=false, mode=:xml)
    ...
    @conn.exec('SELECT * FROM table_societe'+(@debug ? ' ORDER BY so_numero LIMIT 1' : ' WHERE so_numero<=1')).each do |company|
      ...
      if @mode==:xml
        ...
        @file.write "<backup creator=\"Brice\" version=\"20090731080018\">\n"
        @file.write "  <company entity_id=\"#{entity_id}\">\n"
      elsif @mode==:sql
        @file.write "-- version: 20090731080018\n"
      end
      rows(:entities, "SELECT *, #{entity_id} AS thekey, true AS vat_submissive, false AS supplier, true AS reflation_submissive, false AS client, 1 AS language_id, np_numero AS nature_id, true AS active FROM table_societe, table_naturepersonne np WHERE so_numero=#{company_id} AND np_morale LIMIT 1", 'so_libelle'=>'name', 'so_detail'=>'full_name')
      ...
      rows(:languages, "SELECT 1 AS thekey, 'Français' AS native_name, 'French' AS name, 'fr' AS iso2, 'fra' AS iso3 , CURRENT_TIMESTAMP AS created_at, CURRENT_TIMESTAMP AS updated_at, NULL AS created_by, NULL AS updated_by")
      ...
      rows(:accounts, "SELECT 10000 AS thekey, 'Compte banque' AS name, false AS is_debit, '51200001' AS number, true AS usable, true AS groupable, true AS keep_entries, true AS letterable, false AS deleted, '51200001 - Compte banque' AS label, false AS transferable, 0 AS parent_id, true AS pointable #{stamps}")
      rows(:bank_accounts, "SELECT 1 AS thekey, #{entity_id} AS entity_id, 10000 AS account_id, 'Compte courant' AS name, 1 AS currency_id, 'FR76' AS iban, 'FR76' AS iban_label, false AS deleted, 'iban' AS mode, true AS default, jo_numero AS journal_id #{stamps(true)} FROM table_journal WHERE jo_libelle ilike 'b%' and so_numero=#{company_id} LIMIT 1")
      ...
      rows(:accounts, "SELECT *, false AS keep_entries, false AS deleted, cg_numcompte::TEXT||' '||cg_libelle AS label, false AS transferable, 0 AS parent_id FROM table_comptegen WHERE so_numero=#{company_id} @@@", {'cg_libelle'=>'name', 'cg_numero'=>'thekey', 'cg_numcompte'=>'number', 'cg_utilisable'=>'usable', 'cg_debit'=>'is_debit', 'cg_pointable'=>'pointable', 'cg_groupable'=>'groupable', 'cg_lettrable'=>'letterable'})
      ...
      if company_id == 3 or @debug
        ...
        rows(:subscriptions, "SELECT ro.*, NULL AS started_on, NULL AS finished_on, de_numero AS sale_order_id, COALESCE(pd_numero,500000125) AS product_id FROM table_routage ro left join table_facture using (fa_numero) left join table_lignefacture using (fa_numero) @@@", {'ro_numero'=>'thekey', 'ro_debutservice'=>'first_number', 'ro_finservice'=>'last_number', 'ad_numero'=>'contact_id'}) # , 'pe_numero'=>'entity_id'})
      end
      rows(:products, "SELECT *, 0 as amount, pd_id AS number, 1 AS unit_id, 1 AS shelf_id, 'service' AS nature, false AS manage_stocks, true AS to_sale, false AS to_purchase, false AS to_rent, 'buy' AS supply_method, 1 AS critic_quantity_min, 5 AS quantity_min, pd_libelle||' ('||pd_id||')' AS name FROM table_produit WHERE so_numero=#{company_id} @@@", {'pd_numero'=>'thekey', 'pd_titre'=>'catalog_name', 'pd_actif'=>'active', 'pd_id'=>'code'})
      ...
      rows(:sale_orders, "SELECT *, 1 AS expiration_id, 'P' AS state, de_numero AS number, COALESCE(de_acompte, false) AS has_downpayment, 1 AS payment_delay_id, 1 AS nature_id, 'wt' AS sum_method, de_date+'1 month'::INTERVAL AS expired_on, ad_numero AS invoice_contact_id, ad_numero AS delivery_contact_id, ROUND(0.3*de_montantttc, 2) AS downpayment_amount FROM table_devis WHERE so_numero=#{company_id} @@@", {'de_numero'=>'thekey', 'de_libelle'=>'comment', 'pe_numero'=>'client_id', 'de_montantht'=>'amount', 'de_montantttc'=>'amount_with_taxes', 'de_introduction'=>'introduction', 'de_civilites'=>'function_title', 'de_date'=>'created_on', 'de_locked'=>'invoiced', 'de_lettre'=>'letter_format', 'em_numero'=>'responsible_id', 'ad_numero'=>'contact_id'})
      rows(:sale_order_lines, "SELECT l.*, 0 AS position, 1 AS unit_id, cg_numero AS account_id, de_locked AS invoiced FROM table_ligne l JOIN table_devis USING (de_numero) left join table_compteproduit using (pd_numero) WHERE ci_actif AND so_numero=#{company_id} @@@", {'de_numero'=>'order_id', 'l_numero'=>'thekey', 'pd_numero'=>'product_id', 'px_numero'=>'price_id', 'l_quantite'=>'quantity', 'l_montantht'=>'amount', 'l_montantttc'=>'amount_with_taxes'})
      rows(:invoices, "SELECT *, CASE WHEN fa.fa_montantttc>0 THEN 'S' ELSE 'C' END AS nature, 1 AS payment_delay_id, fa_date+'1 month'::INTERVAL AS payment_on, fa_ok AS paid, ROUND(0.3*fa.fa_montantttc, 2) AS downpayment_price FROM table_facture fa left join vue_facture_regle using(fa_numero) WHERE fa.so_numero=#{company_id} @@@", {'fa_numero'=>'thekey', 'de_numero'=>'sale_order_id', 'pe_numero'=>'client_id', 'fa_numfact'=>'number', 'fa_date'=>'created_on', 'fa_avoir_facture'=>'origin_id', 'fa_montantht'=>'amount', 'fa_montantttc'=>'amount_with_taxes', 'fa_perte'=>'lost', 'ad_numero'=>'contact_id', 'fa_avoir'=>'credit', 'fa_accompte'=>'has_downpayment'})
      rows(:invoice_lines, "SELECT l.*, 0 AS position, ld.l_numero AS order_line_id FROM table_lignefacture l join table_facture fa using (fa_numero) join table_ligne ld ON (ld.pd_numero=l.pd_numero AND fa.de_numero=ld.de_numero) WHERE so_numero=#{company_id} @@@", {'lf_numero'=>'thekey', 'fa_numero'=>'invoice_id', 'pd_numero'=>'product_id', 'px_numero'=>'price_id', 'lf_montantht'=>'amount', 'lf_montantttc'=>'amount_with_taxes', 'lf_quantite'=>'quantity'})
      rows(:embankments, "SELECT *, true AS locked, 1 AS bank_account_id FROM table_listereglement JOIN (SELECT lr_numero, count(rg_numero) AS cnt from table_reglement group by 1) as c USING (lr_numero) WHERE so_numero=#{company_id} @@@", {'lr_numero'=>'thekey', 'lr_montant'=>'amount', 'lr_date'=>'created_on', 'lr_commentaire'=>'comment', 'mr_numero'=>'mode_id', 'cnt'=>'payments_number'})
      rows(:payments, "SELECT rg.*, COALESCE(rg.created_at::DATE, rg_date, '0001-01-01'::DATE) AS to_bank_on, false AS downpayment, true AS received, false AS scheduled FROM table_reglement rg WHERE so_numero=#{company_id} @@@", {'rg_numero'=>'thekey', 'pe_numero'=>'entity_id', 'rg_reference'=>'check_number', 'rg_montant'=>'amount', 'mr_numero'=>'mode_id', 'rg_numerocompte'=>'account_number', 'rg_libellebanque'=>'bank', 'lr_numero'=>'embankment_id', 'em_numero'=>'embanker_id', 'rg_date'=>'paid_on'})
      rows(:payment_parts, "SELECT fr.*, COALESCE(de_numero,0) AS order_id FROM table_facturereglement fr join table_reglement rg using (rg_numero) join table_facture using (fa_numero) WHERE rg.so_numero=#{company_id} @@@", {'fr_numero'=>'thekey', 'rg_numero'=>'payment_id', 'fr_montant'=>'amount'})
