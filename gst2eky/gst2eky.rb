...
class Migrator
  ...
  def migrate(debug=false, mode=:xml)
    ...
    @conn.exec('SELECT * FROM table_societe'+(@debug ? ' ORDER BY so_numero LIMIT 1' : ' WHERE so_numero<=1')).each do |company|
      ...
      rows(:entities, "SELECT *, #{maximum_id} AS thekey, 1 AS category_id, true AS vat_submissive, COALESCE(so_detail, so_libelle, '#{company_id}') AS full_name, false AS supplier, true AS reflation_submissive, false AS client, 1 AS language_id, np_numero AS nature_id, true AS active FROM table_societe, table_naturepersonne np WHERE so_numero=#{company_id} AND np_morale LIMIT 1", 'so_libelle'=>'name', 'so_abbrev'=>'code')
      ...
      rows(:accounts, "SELECT #{maximum_id+1} AS thekey, false AS  deleted, false AS groupable, false AS is_debit, false AS keep_entries, '3 Stocks et en-cours' AS label, false AS letterable, 'Stocks et en-cours' AS name, 3 AS number, 0 AS parent_id, false AS pointable, false AS transferable, true AS usable #{stamps}")
      ...
      rows(:payment_modes, "SELECT *, 1 AS bank_account_id, CASE WHEN mr_cheque THEN 'check' ELSE 'other' END AS mode, 'U' AS nature FROM table_modereglement WHERE so_numero=#{company_id} @@@", {'mr_numero'=>'thekey', 'mr_libelle'=>'name', 'cg_numero'=>'account_id', 'mode'=>'mode'})
      if @mode==:xml
        @file.write "  </company>\n"
        @file.write "</backup>"
      end
