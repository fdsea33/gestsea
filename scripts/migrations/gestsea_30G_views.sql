...
CREATE OR REPLACE RULE rule_personne_insert AS
  ...
  DO INSTEAD INSERT INTO "table_personne"(pe_numero, pe_id, tp_numero, np_numero, pe_titre, pe_nom, pe_regimefiscal, pe_actif, deleted, pe_prenom, pe_motdepasse, pe_naissance, pe_numtvaic, created_at, created_by, updated_at, updated_by, lock_version, id) VALUES (new.pe_numero, DEFAULT, new.tp_numero, new.np_numero, new.pe_titre, new.pe_nom, COALESCE(NEW.pe_regimefiscal,'NON RENSEIGNE'), COALESCE(NEW.pe_actif,true), COALESCE(NEW.deleted,false), new.pe_prenom, new.pe_motdepasse, new.pe_naissance, new.pe_numtvaic, CURRENT_TIMESTAMP, CURRENT_USER, CURRENT_TIMESTAMP, CURRENT_USER, 0, DEFAULT);
CREATE OR REPLACE RULE rule_personne_update AS
  ON UPDATE TO "personne"
  DO INSTEAD UPDATE "table_personne" SET pe_numero=new.pe_numero, pe_id=OLD.PE_ID, tp_numero=new.tp_numero, np_numero=new.np_numero, pe_titre=new.pe_titre, pe_nom=new.pe_nom, pe_regimefiscal=COALESCE(NEW.pe_regimefiscal,'NON RENSEIGNE'), pe_actif=COALESCE(NEW.pe_actif,true), deleted=COALESCE(NEW.deleted,false), pe_prenom=new.pe_prenom, pe_motdepasse=new.pe_motdepasse, pe_naissance=new.pe_naissance, pe_numtvaic=new.pe_numtvaic, created_at=OLD.created_at, created_by=OLD.created_by, updated_at=CURRENT_TIMESTAMP, updated_by=CURRENT_USER, lock_version=OLD.lock_version+1, id=OLD.id WHERE new.PE_Numero=PE_Numero;
