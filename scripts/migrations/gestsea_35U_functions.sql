...
BEGIN
  ...
  IF d.so_numero=1 THEN
    ...
    -- Adhésion à 10 €
    IF r=0 THEN
      SELECT count(*) FROM lignefacture JOIN facture f USING (FA_Numero) WHERE f.pe_numero=d.pe_numero AND pd_numero=100051 AND fa_date BETWEEN (a||'-01-01')::DATE AND (a||'-12-31')::DATE INTO c;
      IF c<=0 THEN
